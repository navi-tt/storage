...
import (
	"bytes"
	"fmt"
	"github.com/navi-tt/storage/example/handle"
...
func main() {
	...
	if err := handle.GetToPath("test_fs.txt", "./testdata/test_fs_copy.txt"); err != nil {
		fmt.Printf("err : %s", err.Error())
		return
	}

	if err := handle.Put("test_fs_2.txt", buf, int64(buf.Len())); err != nil {
		fmt.Printf("err : %s", err.Error())
		return
	}

	if err := handle.Del("test_fs_2.txt"); err != nil {
		fmt.Printf("err : %s", err.Error())
		return
	}
}
